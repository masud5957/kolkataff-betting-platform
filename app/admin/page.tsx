...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (!allowed) return
    Promise.all([fetch('/api/admin/recharges'), fetch('/api/admin/withdrawals'), fetch('/api/admin/payment-settings', { cache: 'no-store' })]).then(async ([queue, withdrawalQueue, config]) => {
  ...
  const save = async () => {
    ...
    try {
      const response = await fetch('/api/admin/payment-settings', {
        ...
        body: JSON.stringify({ upiId: settings.upiId, accountName: settings.accountName, qrUrl: settings.qrUrl }),
      })
