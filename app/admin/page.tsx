...
export default function AdminPage() {
  ...
  const refreshQueues = async () => { const [queue, withdrawalQueue] = await Promise.all([fetch('/api/admin/recharges'), fetch('/api/admin/withdrawals')]); if (queue.ok) setRequests((await queue.json()).requests ?? []); if (withdrawalQueue.ok) setWithdrawals((await withdrawalQueue.json()).requests ?? []) }
