...
function AuthScreen({ onAuthenticated }: { onAuthenticated: () => void }) {
  ...
  const submit = async (action = mode) => {
    ...
    try {
      ...
      if (action === 'login') onAuthenticated(); else if (action === 'verify-signup-otp') onAuthenticated(); else { setMessage(result.message); if (action === 'signup' || action === 'resend-verification') { setVerificationPending(true); setResendSeconds(45); window.setTimeout(() => otpInputRef.current?.focus(), 50) } if (action === 'forgot') { setResetOtpPending(true); setResendSeconds(45); window.setTimeout(() => otpInputRef.current?.focus(), 50) } if (action === 'reset') { setMode('login'); setResetOtpPending(false); setOtp(''); setPassword(''); setMessage('Password updated. You can now sign in with your new password.') } }
    ...
    <main className="auth-shell">
      ...
      <section className="auth-panel">
        ...
        <div className="auth-form-wrap">
          <div className="auth-heading"><p className="muted-label">{mode === 'login' ? 'WELCOME BACK' : mode === 'signup' ? 'JOIN KOLKATAFF' : 'ACCOUNT RECOVERY'}</p><h2>{mode === 'login' ? 'Good to see you.' : mode === 'signup' ? 'Make your next move.' : resetOtpPending ? 'Choose a new password.' : 'Reset your password.'}</h2><p>{mode === 'login' ? 'Sign in to access your wallet and games.' : mode === 'signup' ? 'Create your secure KolkataFF account.' : resetOtpPending ? 'Verify the code from your email, then secure your account.' : 'We will send a secure reset code to your inbox.'}</p></div>
          ...
          {!resetOtpPending && <label>Email address<input type="email" autoComplete="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="you@company.com" /></label>}
          ...
          {verificationPending ? <div className="otp-verification-card"><div className="otp-card-icon"><Mail size={20} /></div><strong>Enter your verification code</strong><span>We sent a 6-digit code to <b>{email}</b></span><input ref={otpInputRef} className="otp-input" inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={otp} onChange={event => setOtp(event.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="000000" aria-label="6-digit email verification code" /><button className="primary-action" onClick={() => submit('verify-signup-otp')} disabled={busy || otp.length !== 6}>{busy ? 'Verifying…' : 'Verify email'} <ArrowUpRight size={18} /></button><div className="otp-actions"><button type="button" className="forgot" onClick={() => submit('resend-verification')} disabled={busy || resendSeconds > 0}>{resendSeconds > 0 ? `Resend code in ${resendSeconds}s` : 'Resend code'}</button><button type="button" className="forgot" onClick={() => { setVerificationPending(false); setOtp(''); setError('') }}>Change email</button></div></div> : resetOtpPending ? <div className="otp-verification-card"><div className="otp-card-icon"><LockKeyhole size={20} /></div><strong>Verify your reset request</strong><span>Enter the 6-digit code sent to <b>{email}</b>, then choose a new password.</span><input ref={otpInputRef} className="otp-input" inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={otp} onChange={event => setOtp(event.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="000000" aria-label="6-digit password reset code" /><label>New password<input type="password" autoComplete="new-password" value={password} onChange={event => setPassword(event.target.value)} placeholder="At least 8 characters" /></label><button className="primary-action" onClick={() => submit('reset')} disabled={busy || otp.length !== 6 || password.length < 8}>{busy ? 'Updating password…' : 'Create new password'} <ArrowUpRight size={18} /></button><div className="otp-actions"><button type="button" className="forgot" onClick={() => submit('forgot')} disabled={busy || resendSeconds > 0}>{resendSeconds > 0 ? `Resend code in ${resendSeconds}s` : 'Resend code'}</button><button type="button" className="forgot" onClick={() => { setResetOtpPending(false); setOtp(''); setPassword(''); setError('') }}>Use another email</button></div></div> : <button className="primary-action" onClick={() => submit()} disabled={busy}>{busy ? 'Please wait…' : mode === 'login' ? 'Sign in securely' : mode === 'signup' ? 'Send verification code' : 'Send verification code'} <ArrowUpRight size={18} /></button>}
          {mode === 'forgot' && !resetOtpPending && <button type="button" className="forgot" onClick={() => setMode('login')}>Back to sign in</button>}
...
function Sidebar({ view, setView, onSignOut, userName }: { view: View; setView: (v: View) => void; onSignOut: () => void; userName: string }) {
  return <aside className="sidebar"><div className="side-head"><Logo compact /><span className="live-badge">LIVE</span></div><div className="side-section"><p className="side-label">WORKSPACE</p>{navItems.map(({ label, view: itemView, icon: Icon }) => <button key={itemView} aria-current={view === itemView ? 'page' : undefined} className={view === itemView ? 'side-link active' : 'side-link'} onClick={() => setView(itemView)}><Icon size={18} />{label}{itemView === 'recharge' && <span className="side-plus">+</span>}</button>)}</div><div className="side-section"><p className="side-label">SUPPORT</p><button className="side-link" onClick={() => setView('about')}><CircleHelp size={18} />Help centre</button></div><div className="side-bottom"><div className="profile-mini"><div className="avatar">{userName.slice(0, 2).toUpperCase()}</div><div><strong>{userName}</strong><small>Verified account</small></div><MoreHorizontal size={18} /></div><button className="side-link signout" onClick={onSignOut}><LogOut size={18} />Sign out</button></div></aside>
}
...
function Overview({ setView, balance, totalPlayedPaise, userName, transactions }: { setView: (v: View) => void; balance: number | null; totalPlayedPaise: number; userName: string; transactions: Array<{ id: string; amountPaise: number; note: string; createdAt: string }> }) {
  return <><div className="welcome-row"><div><p className="muted-label">TUESDAY, 18 JUNE 2024</p><h2>Good morning, {userName.split(' ')[0]} <span>✦</span></h2><p className="subtle">Your account is ready. Here&apos;s your latest overview.</p></div><button className="outline-action" onClick={() => setView('recharge')}><ArrowDownToLine size={16} /> Add funds</button></div><div className="stats-grid"><div className="balance-card"><div className="card-top"><span className="muted-label">AVAILABLE BALANCE</span><WalletCards size={19} /></div><strong>{balance === null ? '—' : `₹${(balance / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`}</strong><div className="balance-footer"><span className="positive"><ShieldCheck size={14} /> Live wallet balance</span><span>From backend</span></div></div><div className="stat-card"><span className="muted-label">TOTAL PLAYED</span><strong>₹{(totalPlayedPaise / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</strong><span className="stat-hint">{totalPlayedPaise ? 'Across all bets' : 'No games played yet'}</span></div></div><div className="content-grid"><section className="panel activity-panel"><div className="panel-heading"><div><p className="muted-label">RECENT ACTIVITY</p><h3>Your money, clearly tracked.</h3></div><button className="text-action" onClick={() => setView('activity')}>View all <ArrowUpRight size={15} /></button></div><div className="transaction-list">{transactions.length ? transactions.slice(0, 3).map(tx => <div className="transaction" key={tx.id}><div className={tx.amountPaise >= 0 ? 'tx-icon credit' : 'tx-icon debit'}>{tx.amountPaise >= 0 ? <ArrowDownToLine size={17} /> : <ArrowUpRight size={17} />}</div><div className="tx-info"><strong>{tx.note}</strong><span>{new Date(tx.createdAt).toLocaleString('en-IN')}</span></div><div className="tx-amount"><strong className={tx.amountPaise >= 0 ? 'positive' : ''}>{tx.amountPaise >= 0 ? '+' : '−'}₹{(Math.abs(tx.amountPaise) / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</strong><span><Check size={12} /> Completed</span></div></div>) : <p className="empty-state">No wallet transactions yet.</p>}</div></section><section className="panel quick-panel"><div className="quick-icon"><QrCode size={23} /></div><p className="muted-label">QUICK RECHARGE</p><h3>Top up in seconds.</h3><p>Use UPI, bank transfer or scan a QR. Funds are reviewed securely.</p><button className="primary-action small" onClick={() => setView('recharge')}>Recharge wallet <ArrowUpRight size={16} /></button><div className="payment-marks"><span>UPI</span><span>IMPS</span><span>NEFT</span></div></section></div><section className="notice-bar"><div className="notice-symbol"><ShieldCheck size={19} /></div><div><strong>Your account is fully verified</strong><p>Phone verification complete. You&apos;re all set to play responsibly.</p></div><button onClick={() => setView('profile')}>View profile <ChevronRight size={16} /></button></section></>
}

function PaymentInstructions() { const [settings, setSettings] = useState<{ accountName: string | null; accountNumber: string | null; ifsc: string | null; upiId: string | null; qrUrl: string | null } | null>(null); const [loading, setLoading] = useState(true); const [copied, setCopied] = useState(''); useEffect(() => { fetch('/api/payment-settings', { cache: 'no-store' }).then(r => r.ok ? r.json() : null).then(data => setSettings(data?.settings ?? null)).catch(() => setSettings(null)).finally(() => setLoading(false)) }, []); const copy = async (value: string, label: string) => { await navigator.clipboard.writeText(value); setCopied(label); window.setTimeout(() => setCopied(''), 1600) }; if (loading) return <div className="payment-instructions"><p className="subtle">Loading payment details…</p></div>; if (!settings) return <div className="payment-instructions missing"><strong>Payment details unavailable</strong><span>Our payment team has not configured the receiving account yet. Please try again later.</span></div>; return <div className="payment-instructions"><div className="payment-instructions-heading"><div><p className="muted-label">SECURE PAYMENT</p><h3>Pay KolkataFF</h3><p className="payment-caption">Scan the QR or use the verified UPI ID below.</p></div>{settings.qrUrl && <div className="qr-frame"><img src={settings.qrUrl} alt="KolkataFF payment QR code" className="payment-qr" /><span>Scan to pay</span></div>}</div>{settings.upiId && <div className="upi-highlight"><div><span>PAY USING UPI</span><strong>{settings.upiId}</strong></div><button type="button" onClick={() => copy(settings.upiId!, 'upi')}>{copied === 'upi' ? 'Copied' : 'Copy UPI ID'}</button></div>}<div className="payment-details-heading"><span>BANK TRANSFER DETAILS</span><small>Use only the details shown here</small></div>{settings.accountName && <div className="payment-detail"><span>Account name</span><strong>{settings.accountName}</strong></div>}{settings.accountNumber && <div className="payment-detail"><span>Account number</span><strong>{settings.accountNumber}</strong><button type="button" onClick={() => copy(settings.accountNumber!, 'account')}>{copied === 'account' ? 'Copied' : 'Copy'}</button></div>}{settings.ifsc && <div className="payment-detail"><span>IFSC code</span><strong>{settings.ifsc}</strong><button type="button" onClick={() => copy(settings.ifsc!, 'ifsc')}>{copied === 'ifsc' ? 'Copied' : 'Copy'}</button></div>}<p className="secure-note">Complete your transfer first, then submit the payment reference below. Payments are credited after admin verification.</p></div> }

function Recharge() { const [amount, setAmount] = useState(''); const [submitted, setSubmitted] = useState(false); const [utr, setUtr] = useState(''); const [requests, setRequests] = useState<Array<{ id: string; amountPaise: number; utr: string; status: string; createdAt: string }>>([]); const [error, setError] = useState(''); const [busy, setBusy] = useState(false); useEffect(() => { fetch('/api/wallet/recharge').then(r => r.ok ? r.json() : null).then(data => setRequests(data?.requests ?? [])).catch(() => undefined) }, []); const submit = async () => { setBusy(true); setError(''); try { const response = await fetch('/api/wallet/recharge', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ amount, method: 'upi', utr }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Unable to submit recharge'); setSubmitted(true); setRequests(current => [data.request, ...current]); setAmount(''); setUtr('') } catch (e) { setError(e instanceof Error ? e.message : 'Unable to submit recharge') } finally { setBusy(false) } }; return <div className="recharge-page"><div className="page-intro"><p className="muted-label">WALLET MANAGEMENT</p><h2>Add funds securely.</h2><p>Enter any amount and submit your payment reference. Your recharge is reviewed before being added to your balance.</p></div><div className="recharge-layout"><PaymentInstructions /><section className="panel recharge-card">{submitted && <div className="submission-success" role="status"><strong>Payment submitted successfully</strong><span>Your request is pending admin verification. It will be added to your wallet only after approval.</span></div>}<label className="deposit-amount-field">Deposit amount<input inputMode="decimal" value={amount} onChange={e => setAmount(e.target.value.replace(/[^\d.]/g, ''))} placeholder="Enter any amount" required /><small>Enter the exact amount you transferred.</small></label><div className="step-line second"><span className="step">1</span><div><strong>Payment reference</strong><small>Complete your transfer, then enter the UTR</small></div></div><label>UPI / bank UTR reference<input value={utr} onChange={e => setUtr(e.target.value)} placeholder="Enter the payment reference" /></label>{error && <p role="alert" className="error-message">{error}</p>}<div className="payment-options"><button className="payment-option selected"><div className="method-icon upi">UPI</div><span><strong>UPI / QR code</strong><small>Instant transfer</small></span><Check size={18} /></button><button className="payment-option"><div className="method-icon bank"><CreditCard size={18} /></div><span><strong>Bank transfer</strong><small>NEFT / IMPS</small></span><ChevronRight size={18} /></button></div><button className="primary-action" onClick={submit} disabled={busy || submitted}>Continue to payment <ArrowUpRight size={18} /></button>{submitted && <div className="success-message"><Check size={17} /> Recharge request created for ₹{Number(amount).toLocaleString('en-IN')}. Awaiting admin review.</div>}</section><aside className="panel payment-aside"><div className="qr-placeholder"><QrCode size={88} strokeWidth={1.2} /><span>QR will appear here</span></div><p className="muted-label">SECURE PAYMENTS</p><h3>Every rupee, accounted for.</h3><ul><li><ShieldCheck size={16} /> Manual review on every recharge</li><li><LockKeyhole size={16} /> Your payment details stay private</li><li><Headphones size={16} /> Support when you need it</li></ul></aside></div></div> }

function Withdraw({ balance }: { balance: number | null }) { const [amount, setAmount] = useState(''); const [upiId, setUpiId] = useState(''); const [error, setError] = useState(''); const [message, setMessage] = useState(''); const [busy, setBusy] = useState(false); const [requests, setRequests] = useState<Array<{ id: string; amountPaise: number; utr: string; status: string }>>([]); useEffect(() => { fetch('/api/wallet/withdraw').then(r => r.ok ? r.json() : null).then(data => setRequests(data?.requests ?? [])).catch(() => undefined) }, []); const availableRupees = balance === null ? 0 : Math.floor(balance / 100); const submit = async () => { const requested = Number(amount); if (!Number.isInteger(requested) || requested < 100) { setError('Minimum withdrawal amount is ₹100.'); return } if (balance === null) { setError('Unable to verify your wallet balance. Please try again.'); return } if (requested > availableRupees) { setError(`You can withdraw up to ₹${availableRupees.toLocaleString('en-IN')}, your available balance.`); return } setBusy(true); setError(''); setMessage(''); try { const response = await fetch('/api/wallet/withdraw', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ amount, upiId }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Unable to submit withdrawal'); setMessage('Withdrawal request submitted for admin review.'); setRequests(current => [data.request, ...current]); setAmount(''); } catch (e) { setError(e instanceof Error ? e.message : 'Unable to submit withdrawal') } finally { setBusy(false) } }; return <div className="withdraw-page"><div className="page-intro"><p className="muted-label">WALLET MANAGEMENT</p><h2>Withdraw your winnings.</h2><p>Enter the UPI ID where you want to receive your approved withdrawal.</p></div><section className="panel withdraw-card"><div className="step-line"><span className="step active">1</span><div><strong>Withdrawal amount</strong><small id="withdrawal-limit">Minimum ₹100 · Available: {balance === null ? 'Checking…' : `₹${availableRupees.toLocaleString('en-IN')}`} · Admin approval required</small></div></div><div className="amount-input"><span>₹</span><input inputMode="numeric" value={amount} min={100} max={availableRupees} onChange={e => setAmount(e.target.value.replace(/\D/g, '').slice(0, 7))} placeholder="0" aria-describedby="withdrawal-limit" /></div><div className="step-line second"><span className="step">2</span><div><strong>Receiving UPI ID</strong><small>Double-check this before submitting</small></div></div><label>UPI ID<input value={upiId} onChange={e => setUpiId(e.target.value)} placeholder="yourname@upi" /></label>{error && <p role="alert" className="error-message">{error}</p>}{message && <p className="secure-note">{message}</p>}<button className="primary-action" onClick={submit} disabled={busy}>{busy ? 'Submitting…' : 'Request withdrawal'} <ArrowUpRight size={18} /></button></section></div> }

function ResultBoard({ rounds, bets, onRefresh }: { rounds: Array<{ id: string; roundDate?: string; roundNumber: number; status: string; singleResult: string | null; pattiResult: string | null }>; bets: Array<{ roundId: string; roundNumber: number; betType: string; selection: string; stakePaise: number; status: string; payoutPaise: number }>; onRefresh: () => void }) {
  const grouped = rounds.reduce<Record<string, typeof rounds>>((groups, round) => { const key = round.roundDate ?? 'today'; (groups[key] ??= []).push(round); return groups }, {})
  const labelDate = (date: string) => date === 'today' ? 'TODAY' : new Date(`${date}T12:00:00`).toLocaleDateString('en-IN', { day: '2-digit', month: '2-digit', year: 'numeric' })
  return <section className="results-board-wrap"><div className="live-result-banner"><span className="live-dot" /> LIVE <strong>Kolkata Fatafat result updates</strong></div><div className="results-board"><div className="result-date-bar">• {labelDate(Object.keys(grouped)[0] ?? 'today')}</div><div className="result-grid">{(grouped[Object.keys(grouped)[0] ?? 'today'] ?? []).slice().sort((a, b) => a.roundNumber - b.roundNumber).map(round => <div className="result-cell" key={round.id}><strong>{round.pattiResult ?? '—'}</strong><span>{round.singleResult ?? '—'}</span></div>)}</div></div><button className="refresh-results" type="button" onClick={onRefresh}>Refresh results</button>{Object.keys(grouped).length > 1 && <><h3 className="old-results-heading">OLD RESULTS</h3>{Object.entries(grouped).slice(1).map(([date, dateRounds]) => <div className="results-board old-board" key={date}><div className="result-date-bar">{labelDate(date)}</div><div className="result-grid">{dateRounds.slice().sort((a, b) => a.roundNumber - b.roundNumber).map(round => <div className="result-cell" key={round.id}><strong>{round.pattiResult ?? '—'}</strong><span>{round.singleResult ?? '—'}</span></div>)}</div></div>)}</>}<div className="my-bets-board"><div className="panel-heading"><div><p className="muted-label">MY BETS</p><h3>What you played</h3></div></div>{bets.length ? bets.slice(0, 8).map(bet => <div className="my-bet-row" key={`${bet.roundId}-${bet.betType}-${bet.selection}`}><span>R{bet.roundNumber} · {bet.betType === 'patti' ? 'Patti' : 'Single'} <strong>{bet.selection}</strong></span><span>₹{(bet.stakePaise / 100).toLocaleString('en-IN')} · {bet.status}</span></div>) : <p className="subtle">Your bets will appear here after you play.</p>}</div></section>
}

function Game() {
  const [rounds, setRounds] = useState<Array<{ id: string; roundDate?: string; roundNumber: number; status: string; singleResult: string | null; pattiResult: string | null }>>([])
  const [todayRounds, setTodayRounds] = useState<Array<{ id: string; roundNumber: number; status: string; singleResult: string | null; pattiResult: string | null }>>([])
  const [bets, setBets] = useState<Array<{ roundId: string; roundNumber: number; betType: string; selection: string; stakePaise: number; status: string; payoutPaise: number }>>([])
  const [roundId, setRoundId] = useState('')
  const [betType, setBetType] = useState<'single' | 'patti'>('single')
  const [selection, setSelection] = useState('')
  const [stake, setStake] = useState('500')
  const [message, setMessage] = useState('')
  const [error, setError] = useState('')
  const [busy, setBusy] = useState(false)
  const loadResults = async () => { try { const [historyResponse, todayResponse, betResponse] = await Promise.all([fetch('/api/game/rounds?history=1', { cache: 'no-store' }), fetch('/api/game/rounds', { cache: 'no-store' }), fetch('/api/game/bets', { cache: 'no-store' })]); const historyData = historyResponse.ok ? await historyResponse.json() : null; const todayData = todayResponse.ok ? await todayResponse.json() : null; const betData = betResponse.ok ? await betResponse.json() : null; const history = historyData?.rounds ?? []; const currentRounds = todayData?.rounds ?? []; setRounds(history); setTodayRounds(currentRounds); setBets(betData?.bets ?? []); const today = currentRounds.find((round: { status: string }) => round.status === 'open'); setRoundId(today?.id ?? currentRounds[0]?.id ?? '') } catch { setError('Unable to load result history.') } }
  useEffect(() => { loadResults() }, [])
  const placeBet = async () => {
    setBusy(true); setError(''); setMessage('')
    try { const response = await fetch('/api/game/bets', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ roundId, betType, selection, stake }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Unable to place bet.'); await loadResults(); setMessage('Bet placed. Your wallet has been debited securely.'); setSelection('') } catch (err) { setError(err instanceof Error ? err.message : 'Unable to place bet.') } finally { setBusy(false) }
  }
  return <div className="game-page"><div className="page-intro"><p className="muted-label">KOLKATAFF GAME DESK</p><h2>Choose your number.</h2><p>Eight admin-created rounds run each day. Place your single or exact patti bet before the result is declared.</p></div><ResultBoard rounds={rounds} bets={bets} onRefresh={loadResults} /><div className="game-layout"><section className="panel game-card"><div className="panel-heading"><div><p className="muted-label">PLACE A BET</p><h3>Pick your play</h3></div><span className="live-badge">₹500–₹10,000</span></div><label>Today&apos;s round<select value={roundId} onChange={event => setRoundId(event.target.value)}>{todayRounds.map(round => <option key={round.id} value={round.id}>Round {round.roundNumber} · {round.status === 'open' ? 'Open' : round.status}</option>)}</select></label><div className="bet-type-toggle"><button type="button" className={betType === 'single' ? 'active' : ''} onClick={() => { setBetType('single'); setSelection('') }}>Single <small>0–9 · 9×</small></button><button type="button" className={betType === 'patti' ? 'active' : ''} onClick={() => { setBetType('patti'); setSelection('') }}>Patti <small>000–999 · 100×</small></button></div><label>{betType === 'single' ? 'Single digit' : 'Exact three-digit patti'}<input inputMode="numeric" maxLength={betType === 'single' ? 1 : 3} value={selection} onChange={event => setSelection(event.target.value.replace(/\D/g, '').slice(0, betType === 'single' ? 1 : 3))} placeholder={betType === 'single' ? '0' : '000'} /></label><label>Stake amount<input inputMode="numeric" value={stake} onChange={event => setStake(event.target.value.replace(/\D/g, ''))} /><small>Minimum ₹500 · Maximum ₹10,000</small></label>{message && <p className="secure-note" role="status">{message}</p>}{error && <p className="error-message" role="alert">{error}</p>}<button className="primary-action" type="button" onClick={placeBet} disabled={busy || !roundId || selection.length !== (betType === 'single' ? 1 : 3)}>{busy ? 'Placing bet…' : `Place ${betType} bet`}</button></section><section className="panel results-card"><div className="panel-heading"><div><p className="muted-label">RESULT BOARD</p><h3>Today&apos;s declared results</h3></div><ClipboardList size={20} /></div>{rounds.length === 0 ? <p className="subtle">Rounds will appear when the admin creates today&apos;s game.</p> : <div className="round-list">{rounds.map(round => <div className="round-row" key={round.id}><span>Round {round.roundNumber}</span>{round.status === 'declared' ? <strong>{round.singleResult} · {round.pattiResult}</strong> : <em>Awaiting result</em>}</div>)}</div>}</section></div></div>
}
...
function Activity() {
  const [items, setItems] = useState<Array<{ kind: string; amountPaise: number; reference: string | null; status: string; createdAt: string }>>([])
  useEffect(() => { fetch('/api/wallet/transactions').then(r => r.ok ? r.json() : null).then(data => setItems(data?.items ?? [])).catch(() => undefined) }, [])
  return <div className="activity-page"><div className="page-intro"><p className="muted-label">LEDGER</p><h2>Transaction history.</h2><p>A clear record of every wallet movement.</p></div><section className="panel table-panel"><div className="table-toolbar"><div className="filter-tabs"><button className="active">All activity</button><button>Recharges</button><button>Transfers</button></div><button className="outline-action"><ArrowDownToLine size={15} /> Export</button></div><div className="table-wrap"><table><thead><tr><th>DESCRIPTION</th><th>DATE</th><th>METHOD</th><th>AMOUNT</th><th>STATUS</th></tr></thead><tbody>{items.map((tx, i) => <tr key={i}><td><strong>{tx.kind === 'withdrawal' ? 'Withdrawal' : 'Recharge'}</strong><small>{tx.reference}</small></td><td>{new Date(tx.createdAt).toLocaleDateString('en-IN')}</td><td>{tx.kind === 'withdrawal' ? 'UPI' : 'Payment'}</td><td className={tx.amountPaise >= 0 ? 'positive' : ''}>{tx.amountPaise >= 0 ? '+' : '−'}₹{(Math.abs(tx.amountPaise) / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td><td><span className={`status-pill status-${tx.status}`}><Check size={12} /> {tx.status}</span></td></tr>)}</tbody></table></div></section></div> }
...
export default function Page() {
  ...
  const [totalPlayedPaise, setTotalPlayedPaise] = useState(0)
  ...
  setTotalPlayedPaise(walletData?.totalPlayedPaise ?? 0)
  ...
  const titles: Record<View, string> = { overview: 'Overview', game: 'Play today', recharge: 'Recharge wallet', withdraw: 'Withdraw funds', activity: 'Activity', profile: 'Profile & settings', about: 'About KolkataFF' }
  ...
  return <main className="app-shell"><div className={mobileOpen ? 'sidebar-wrap open' : 'sidebar-wrap'}><Sidebar view={view} setView={navigateTo} onSignOut={signOut} userName={user?.name ?? 'Player'} />{mobileOpen && <button className="drawer-close" onClick={() => setMobileOpen(false)} aria-label="Close menu"><X /></button>}</div><div className="main-area"><Topbar title={titles[view]} onMenu={() => setMobileOpen(true)} userName={user?.name ?? 'Player'} /><div className="page-content">{view === 'overview' && <Overview setView={navigateTo} balance={balance} totalPlayedPaise={totalPlayedPaise} userName={user?.name ?? 'Player'} transactions={transactions} />}{view === 'game' && <Game />}{view === 'recharge' && <Recharge />}{view === 'withdraw' && <Withdraw balance={balance} />}{view === 'activity' && <Activity />}{view === 'profile' && <Profile />}{view === 'about' && <About />}</div></div></main>
}
