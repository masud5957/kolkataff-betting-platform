...
export default function Page() {
  ...
  return <main className="app-shell"><div className={mobileOpen ? 'sidebar-wrap open' : 'sidebar-wrap'}><Sidebar view={view} setView={navigateTo} onSignOut={signOut} userName={user?.name ?? 'Player'} />{mobileOpen && <button className="drawer-close" onClick={() => setMobileOpen(false)} aria-label="Close menu"><X /></button>}</div><div className="main-area"><Topbar title={titles[view]} onMenu={() => setMobileOpen(true)} userName={user?.name ?? 'Player'} /><div className="page-content">{view === 'overview' && <Overview setView={navigateTo} balance={balance} userName={user?.name ?? 'Player'} transactions={transactions} />}{view === 'game' && <Game />}{view === 'recharge' && <Recharge />}{view === 'withdraw' && <Withdraw balance={balance} />}{view === 'activity' && <Activity />}{view === 'profile' && <Profile />}{view === 'about' && <About />}</div></div></main>
}
