...
export default function Page() {
  ...
  return <main className="app-shell"><div className={mobileOpen ? 'sidebar-wrap open' : 'sidebar-wrap'}><Sidebar view={view} setView={v => { setView(v); setMobileOpen(false) }} onSignOut={signOut} userName={user?.name ?? 'Player'} />{mobileOpen && <button className="drawer-close" onClick={() => setMobileOpen(false)} aria-label="Close menu"><X /></button>}</div><div className="main-area"><Topbar title={titles[view]} onMenu={() => setMobileOpen(true)} userName={user?.name ?? 'Player'} /><div className="page-content">{view === 'overview' && <Overview setView={setView} balance={balance} userName={user?.name ?? 'Player'} transactions={transactions} />}{view === 'recharge' && <Recharge />}{view === 'activity' && <Activity />}{view === 'profile' && <Profile />}</div></div></main>
}
