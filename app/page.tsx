...
function Game() {
  ...
  const [error, setError] = useState('')
  const [busy, setBusy] = useState(false)
  const loadResults = () => { Promise.all([fetch('/api/game/rounds?history=1', { cache: 'no-store' }), fetch('/api/game/bets', { cache: 'no-store' })]).then(async ([roundResponse, betResponse]) => { const roundData = roundResponse.ok ? await roundResponse.json() : null; const betData = betResponse.ok ? await betResponse.json() : null; const history = roundData?.rounds ?? []; setRounds(history); setBets(betData?.bets ?? []); const today = history.find((round: { status: string }) => round.status === 'open'); setRoundId(today?.id ?? history[0]?.id ?? '') }).catch(() => setError('Unable to load result history.')) }
