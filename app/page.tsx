...
type View = 'overview' | 'recharge' | 'activity' | 'profile'
...
export default function Page() {
  ...
  const titles: Record<View, string> = { overview: 'Overview', recharge: 'Recharge wallet', activity: 'Activity', profile: 'Profile & settings' }
  const signOut = async () => { await fetch('/api/auth/logout', { method: 'POST' }); setAuthed(false); setUser(null) }
  return <main className="app-shell"><div className={mobileOpen ? 'sidebar-wrap open' : 'sidebar-wrap'}><Sidebar view={view} setView={v => { setView(v); setMobileOpen(false) }} onSignOut={signOut} />{mobileOpen && <button className="drawer-close" onClick={() => setMobileOpen(false)} aria-label="Close menu"><X /></button>}</div><div className="main-area"><Topbar title={titles[view]} onMenu={() => setMobileOpen(true)} /><div className="page-content">{view === 'overview' && <Overview setView={setView} balance={balance} userName={user?.name ?? 'Player'} />}{view === 'recharge' && <Recharge />}{view === 'activity' && <Activity />}{view === 'profile' && <Profile />}</div></div></main>
}
