...
function Game() {
  const [rounds, setRounds] = useState<Array<{ id: string; roundNumber: number; status: string; singleResult: string | null; pattiResult: string | null }>>([])
  ...
  const [error, setError] = useState('')
  const [busy, setBusy] = useState(false)
  useEffect(() => { fetch('/api/game/rounds', { cache: 'no-store' }).then(response => response.ok ? response.json() : null).then(data => { const next = data?.rounds ?? []; setRounds(next); setRoundId(next.find((round: { status: string }) => round.status === 'open')?.id ?? next[0]?.id ?? '') }).catch(() => setError('Unable to load today\'s rounds.')) }, [])
