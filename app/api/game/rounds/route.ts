...
export async function GET() {
  ...
  const rounds = await db.select({ id: gameRounds.id, roundNumber: gameRounds.roundNumber, status: gameRounds.status, singleResult: gameRounds.singleResult, pattiResult: gameRounds.pattiResult, createdAt: gameRounds.createdAt, declaredAt: gameRounds.declaredAt }).from(gameRounds).where(eq(gameRounds.roundDate, date)).orderBy(desc(gameRounds.roundNumber))
