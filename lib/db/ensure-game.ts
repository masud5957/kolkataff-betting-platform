...
export function ensureGameTables() {
  if (!ready) {
    ready = (async () => {
      await pool.query(`CREATE TABLE IF NOT EXISTS game_rounds (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), round_date text NOT NULL, round_number integer NOT NULL, status text NOT NULL DEFAULT 'open', single_result text, patti_result text, created_by uuid NOT NULL, declared_at timestamptz, created_at timestamptz NOT NULL DEFAULT now(), UNIQUE(round_date, round_number))`)
