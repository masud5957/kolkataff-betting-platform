...
const statements = [
  ...
  'create table if not exists recharge_requests (id uuid primary key default gen_random_uuid(), user_id uuid not null, amount_paise integer not null, method text not null, upi_id text, account_name text, account_number text, ifsc_code text, utr text not null unique, proof_url text, status text not null default \'pending\', reviewed_by uuid, reviewed_at timestamptz, created_at timestamptz not null default now())',
  'alter table recharge_requests add column if not exists upi_id text',
  'alter table recharge_requests add column if not exists account_name text',
  'alter table recharge_requests add column if not exists account_number text',
  'alter table recharge_requests add column if not exists ifsc_code text',
...
export function ensureDatabase() {
  if (!bootstrapPromise) {
    bootstrapPromise = (async () => {
      ...
      try {
        await client.query('select pg_advisory_lock(hashtext(\'kolkataff_schema\'))')
        for (const statement of statements) await client.query(statement)
        await client.query('select pg_advisory_unlock(hashtext(\'kolkataff_schema\'))')
      } finally {
        client.release()
      }
    })().catch(error => {
